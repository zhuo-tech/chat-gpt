        ...
        <div v-show="showPlaceholder" :class="placeholderClass" class="absolute-lt w-full h-full">
            ...
            <div v-show="isEmpty" class="absolute-center">
                <div class="relative">
                    ...
                    <p :class="descClass" class="absolute-lb w-full text-center">{{ emptyDesc }}</p>
                </div>
            </div>
            <div v-show="!network" class="absolute-center">
                <div :class="{ 'cursor-pointer': showNetworkReload }" class="relative" @click="handleReload">
                    <icon-local-network-error :class="iconClass" />
                    <p :class="descClass" class="absolute-lb w-full text-center">{{ networkErrorDesc }}</p>
                </div>
            </div>
        </div>
    </div>

<template>
    <div class="wh-full flex-col-center">
        <n-gradient-text :size="28" class="mb-24px" type="primary">Custom Constant Page</n-gradient-text>
        <router-link :to="{ name: routeHomePath }">
            <n-button type="primary">回到首页</n-button>
        </router-link>
        <n-card :bordered="false" class="mt-24px rounded-16px shadow-sm" size="small">
            <div class="flex-center py-12px">
                <n-button :disabled="isMoving" class="mr-24px" type="primary" @click="startMove">开始</n-button>
                <n-button type="error" @click="endMove">暂停</n-button>
            </div>
            <div class="flex-center">
                <div :style="containerStyle" class="relative bg-primary_active">
                    ...
                    <div :style="transformStyle" class="absolute left-182px top-82px w-40px h-40px bg-red rounded-20px"></div>
                </div>
            </div>
        </n-card>
    </div>
